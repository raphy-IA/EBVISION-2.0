...
// Chercher une partie unique du code existant
const searchPart = `const submittableStatuses = ['draft', 'saved', 'rejected'];`;
const insertionPoint = content.indexOf(searchPart);

if (insertionPoint !== -1) {
    // Trouver la fin du bloc if qui suit
    const ifBlockStart = content.indexOf('if', insertionPoint);
    const ifBlockEnd = content.indexOf('}', ifBlockStart) + 1;
    ...
    const newContent = content.slice(0, ifBlockEnd) + codeToInsert + content.slice(ifBlockEnd);
