...
async function cleanupAllDuplicatePermissions() {
    const client = await pool.connect();

    try {
        await client.query('BEGIN');

        console.log('🔍 Nettoyage complet de TOUTES les permissions en double...\n');

        // Trouver TOUTES les permissions avec le même nom (toutes catégories confondues)
        ...
        for (const dup of duplicatesResult.rows) {
            categoriesProcessed.add(dup.category);
            console.log(`\n📝 [${dup.category}] "${dup.name}" (${dup.count} occurrences)`);
        ...
        console.log(`   - Catégories traitées: ${Array.from(categoriesProcessed).join(', ')}`);
...
cleanupAllDuplicatePermissions();
