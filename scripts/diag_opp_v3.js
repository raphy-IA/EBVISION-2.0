...
async function checkOpportunities() {
    ...
    try {
        console.log('--- Diagnostic des Opportunités & Années Fiscales v3 ---');
        ...
        fyCountsRes.rows.forEach(r => {
            console.log(`- ${r.libelle || 'SANS LIBELLÉ'} (${r.annee || 'SANS ANNÉE'}) : ${r.count}`);
        });
        ...
        fyDatesRes.rows.forEach(r => {
            console.log(`- ${r.libelle} (${r.annee}) : ${r.date_debut} au ${r.date_fin} [ID: ${r.id}]`);
        });
