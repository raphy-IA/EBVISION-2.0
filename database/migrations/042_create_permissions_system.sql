-- Migration pour le système de permissions EB-Vision 2.0
-- Date: 2025-08-29
-- Description: Création du système d'habilitation complet

-- 1. Table des rôles système
...
-- 4. Table des permissions utilisateur individuelles (override)
...
-- 5. Table des accès BU par utilisateur
...
-- 6. Table d'audit des permissions (optionnel)
CREATE TABLE IF NOT EXISTS permission_audit_log (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID REFERENCES users(id),
    action VARCHAR(50) NOT NULL, -- 'GRANT', 'REVOKE', 'MODIFY'
    target_type VARCHAR(50) NOT NULL, -- 'ROLE', 'PERMISSION', 'BU_ACCESS'
    target_id UUID,
    details JSONB,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- 7. Ajout d'une colonne role_id à la table users pour la migration
