-- Peuplement du système de permissions simple
-- Date: 2025-08-29
...
UPDATE users 
SET role_id = (SELECT id FROM roles WHERE name = 'CONSULTANT')
WHERE role = 'CONSULTANT' AND role_id IS NULL;

-- Assigner le rôle COLLABORATEUR par défaut aux autres utilisateurs
UPDATE users 
SET role_id = (SELECT id FROM roles WHERE name = 'COLLABORATEUR')
