...
async function restoreBusinessUnit() {
    try {
        console.log('🔧 Restauration de la Business Unit supprimée...\n');
        ...
        const checkResponse = await makeRequest(`/api/business-units/${buId}`);
        ...
        const restoreResponse = await makeRequest(`/api/business-units/${buId}`, 'PUT', restoreData);
        ...
        const finalCheck = await makeRequest('/api/business-units?limit=100');
