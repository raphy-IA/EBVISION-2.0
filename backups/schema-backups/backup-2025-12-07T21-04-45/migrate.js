...
async function executeMigration(filename) {
    ...
    console.log('SQL Content:', content); // Debug
    ...
    try {
        ...
        // Gérer les erreurs de type "relation déjà existante" ou "colonne déjà existante"
        // 42P07: duplicate_table
        // 42701: duplicate_column
        // 42710: duplicate_object (constraint, index, etc.)
        const IGNORED_ERRORS = ['42P07', '42701', '42710'];

        if (IGNORED_ERRORS.includes(error.code)) {
            log(`⚠️  Migration ignorée (déjà appliquée partiellement): ${filename}`, 'yellow');
            log(`   Raison: ${error.message}`, 'gray');

            // Marquer comme exécutée pour ne pas bloquer les suivantes
            await client.query(
        ...
        if (error.stack) log(error.stack, 'red'); // Log stack trace
        throw error;
    } finally {
        client.release();
    }
